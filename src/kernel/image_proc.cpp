...
static inline float clamp_upper1(float x) {
    return (x > 1.0f) ? 1.0f : x;
}

static inline float image_proc_one(float r_in,
                                   float g_in,
                                   float b_in,
                                   float threshold) {
    constexpr float kGain = 1.05f;
    constexpr float kShift = 0.02f;
    constexpr float kGrayR = 0.299f;
    constexpr float kGrayG = 0.587f;
    constexpr float kGrayB = 0.114f;
    constexpr float kAdjBias = 0.05f;
    constexpr float kAdjScale = 1.1111111111f; // 1 / 0.90
    ...
    float r_val = clamp_upper1(r_in * kGain + kShift);
    float g_val = clamp_upper1(g_in * kGain + kShift);
    float b_val = clamp_upper1(b_in * kGain + kShift);

    const float gray = r_val * kGrayR + g_val * kGrayG + b_val * kGrayB;
    const float adjusted = clamp01((gray - kAdjBias) * kAdjScale);
    const float gray_enhance = adjusted * adjusted * (3.0f - 2.0f * adjusted);

    const float intensity = gray_enhance * 1.2f;
    const float half = intensity * 0.5f;
    const float gain_base = std::sqrt(half * half + 0.1f);
    const float gain =
    ...
    mask = clamp01(mask * 0.7f + noise * 0.3f);
    ...
    return clamp01(compress_val * weight);
}
...
void stu_image_proc(image_proc_args& args) {
    ...
    size_t i = 0;
    for (; i + 1 < n; i += 2) {
        out[i] = image_proc_one(r[i], g[i], b[i], threshold);
        out[i + 1] = image_proc_one(r[i + 1], g[i + 1], b[i + 1], threshold);
    }
    for (; i < n; ++i) {
        out[i] = image_proc_one(r[i], g[i], b[i], threshold);
    }
