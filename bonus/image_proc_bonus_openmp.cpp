#include "image_proc_bonus_openmp.h"
...
static inline float small_sin_bonus(float x) {
    const float x2 = x * x;
    return x * (1.0f - x2 * (1.0f / 6.0f));
}

static inline float small_cos_bonus(float x) {
    const float x2 = x * x;
...
void bonus_image_proc_openmp(image_proc_args& args) {
...
#pragma omp parallel for schedule(static)
    ...
    for (std::ptrdiff_t idx = 0; idx < static_cast<std::ptrdiff_t>(n); ++idx) {
        float r_val = r[idx] * 1.05f + 0.02f;
        float g_val = g[idx] * 1.05f + 0.02f;
        float b_val = b[idx] * 1.05f + 0.02f;
        ...
        const float noise =
            small_sin_bonus(compress_val * p0) * small_cos_bonus(r_val * p1);
        ...
        const float weight = (lut_idx < 4)
                                 ? (lut[lut_idx] * (1.0f - frac) +
                                    lut[lut_idx + 1] * frac)
                                 : lut[4];
...
void bonus_image_proc_openmp_wrapper(void* ctx) {
    auto& args = *static_cast<image_proc_args*>(ctx);
    bonus_image_proc_openmp(args);
}
